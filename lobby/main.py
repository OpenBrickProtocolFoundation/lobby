...
def start_gameserver(lobby_id: str) -> tuple[Response, HTTPStatus]:
    ...
    with active_lobbies.lock() as locked:
        ...
        env = os.environ.copy()
        env["PATH"] = current_app.config[ConfigValue.SIMULATOR_LIBRARY_PATH.value]
        subprocess.Popen([current_app.config[ConfigValue.GAMESERVER_EXECUTABLE.value], socket_port], env=env)
        # print(f"manually start the gameserver on port {socket_port}")
...
def unregister() -> tuple[Response, HTTPStatus]:
    ...
    try:
        ...
        return create_error_response("Cannot delete non-existing user.", HTTPStatus.CONFLICT)
